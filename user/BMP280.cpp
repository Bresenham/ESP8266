...
void BMP280::setup(void) {
    ...
    if(sensor_id == 0x00)
        ...
        if(read_reg(SENSOR_CTRL_MEAS_REG) == ctrl_meas_reg_data) {
            ...
            write_reg(SENSOR_CONFIG_REG, SENSOR_STANDBY_TIME_2000_MS);
            os_delay_us(100);

            if(read_reg(SENSOR_CONFIG_REG) == SENSOR_STANDBY_TIME_2000_MS) {
                os_printf("SUCCESSFULLY CONFIGURED: STANDBY TIME 2000MS.\r\n");
